...
def main():
    ...
    with col1:
        ...
        # Chat input
        # Check if there's an emoji to add from the emoji picker
        emoji_placeholder = ""
        if hasattr(st.session_state, 'emoji_text_to_add') and st.session_state.emoji_text_to_add:
            emoji_placeholder = st.session_state.emoji_text_to_add
            # Clear after retrieving
            st.session_state.emoji_text_to_add = ""
        ...
        if user_input := st.chat_input("Message the AI...", key="chat_input"):
            ...
            # If we have an emoji from the picker, add it to the message
            if emoji_placeholder:
                user_input = emoji_placeholder + " " + user_input
